...
def main():
    ...
    file_path_save = 'data/gannon_20241006_' + str(t_min) + 'm_ds.pkl'
    with open(file_path, 'rb') as f:
        data = pkl.load(f)

    output_dir = file_path_save[:-4] + "_2D_owd"
    ...
    lat_list = data['lat']    # list of lists
    tvec = data['tvec']
    ...
    A_global = np.zeros((m, nt*nlat))
    counts = np.zeros((nt, nlat))
    for i in tqdm(range(m), desc="Building Global A"):
        ...
        counts[t_bins, lat_bins_i] += 1
        A_global[i, t_bins * nlat + lat_bins_i] = 1.0 / len(t_bins) if len(t_bins) > 0 else 0

    # convert A_global to sparse matrix
    A_global = csr_matrix(A_global)

    # === A_global: measurement-to-(t,lat) bin mapping ===
    # A_global = csr_matrix((np.ones(m), (np.arange(m), time_idx_all * nlat + lat_idx_all)), shape=(m, nt * nlat))
    # A_global = csr_matrix((np.ones(m_expanded), (np.arange(m_expanded), lat_idx_all * nlat + time_idx_all)), shape=(m_expanded, nt * nlat))
    
    # === Initialization ===
    h0 = 600
    n_iter = 3
    rho0_vec = np.ones(nt * nlat) * 1e-10
    H_vec = np.ones(nt * nlat) * 100.0

    # L_t = build_laplacian(nt, weight=100).toarray()
    # L_lat = build_laplacian(nlat, weight=10).toarray()
    # L_H = build_laplacian_2d(nt, nlat, weight_t=100, weight_lat=1)
    # A_H = vstack([np.eye(nt * nlat), L_H])

    # L_rho = csr_matrix(np.kron(np.eye(nlat), L_t) + np.kron(L_lat, np.eye(nt)))
    L_rho = build_laplacian_2d(nt, nlat, weight_t=50, weight_lat=0.5)  # adjust weights as needed
    ...
    for iteration in range(n_iter):
        ...
        H_vec = smooth_H_box(H_vec_new.reshape(nt, nlat), size=(20, 5)).reshape(nt * nlat) # 20,5

        # H_vec = H_vec_new.copy()
        # b_H = np.concatenate([H_vec_new, np.zeros(L_H.shape[0])])
        # H_vec = lsqr(A_H, b_H)[0]
        # H_vec = smooth_log_H(H_vec_new.reshape(nlat, nt), window=10).reshape(nt * nlat)
        # H_vec = smooth_log_H(H_vec_new.reshape(nt, nlat), window=10).reshape(nt * nlat)
        # H_vec = H_vec_new
    ...
    results = {
        'tvec': tvec,
        'lat_bins': lat_bins,
        'rho0_vec': rho0_vec.reshape(nt, nlat),
        'H_vec': H_vec.reshape(nt, nlat),
    ...
    # # figure out the counts in each lat/time bin. If the count is < 10, set it to nan
    # counts = np.zeros((nlat, nt))
    # for i in range(m):
    #     t_bins = time_idx[i]
    #     lat_bins_i = lat_idx[i]
    #     counts[lat_bins_i, t_bins] += 1
    ...
    plt.subplot(2, 1, 1)
    plt.imshow(results['rho0_vec'].T, aspect='auto', origin='lower', extent=[tvec[0], tvec[-1], lat_bins[0], lat_bins[-1]], cmap='jet')
    ...
    plt.ylabel('Latitude (°)')
    plt.subplot(2, 1, 2)
    plt.imshow(results['H_vec'].T, aspect='auto', origin='lower', extent=[tvec[0], tvec[-1], lat_bins[0], lat_bins[-1]], cmap='jet')
    ...
    plt.imshow(dens_2d, aspect='auto', origin='lower', extent=[tvec[0], tvec[-1], lat_bins[0], lat_bins[-1]], cmap='jet')
    plt.colorbar(label='Density (kg/m³)')
